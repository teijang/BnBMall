...
namespace Nop.Web.Factories
{
    ...
    public partial class ProductModelFactory : IProductModelFactory
    {
        ...
        private string videoBucketCDNName;
        ...
        public ProductModelFactory(CaptchaSettings captchaSettings,
            ...
            videoBucketCDNName = "http://" + config.AWSS3ReviewVideoBucketCDN;
        }
        ...
        public virtual ProductReviewsModel PrepareProductReviewsModel(ProductReviewsModel model, Product product) {
            ...
            foreach(var pr in productReviews) {
                ...
                var reviewImageIds = _genericAttributeService.GetAttribute<string>(pr, "ReviewImageId");
                var reviewVideos = _genericAttributeService.GetAttribute<string>(pr, "ReviewVideos");
                var imageIds = new List<string>();
                var videos = new List<string>();
                if(!string.IsNullOrEmpty(reviewImageIds)) {
                    imageIds = reviewImageIds.Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries).Select(x => x.Trim()).Where(x => x != "0").ToList();
                }
                if(!string.IsNullOrEmpty(reviewVideos)) {
                    videos = reviewVideos.Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries).Select(x => x.Trim()).Where(x => !string.IsNullOrEmpty(x)).ToList();
                }
                ...
                foreach(var item in imageIds) {
                    var imageId = Convert.ToInt32(item.Trim());
                    productReviewModel.ImageUrls.Add(_pictureService.GetReviewPictureUrl(imageId, 200));
                }

                foreach(var item in videos) {
                    productReviewModel.VideoUrls.Add(videoBucketCDNName + "/" + item);
                }
            ...
            model.AddProductReview.ReviewProductId = product.Id;

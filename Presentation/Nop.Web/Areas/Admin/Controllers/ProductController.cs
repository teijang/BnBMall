...
namespace Nop.Web.Areas.Admin.Controllers
{
    public partial class ProductController : BaseAdminController
    {
        ...
        protected virtual void UpdateLocales(ProductContentType pct, ProductContentTypeModel model) {
            foreach(var localized in model.Locales) {
                _localizedEntityService.SaveLocalizedValue(pct,
                    x => x.Name,
                    localized.Name,
                    localized.LanguageId);
            }
        ...
        protected virtual void UpdateLocales(ProductContent pc, ProductContentModel model) {
            foreach(var localized in model.Locales) {
                _localizedEntityService.SaveLocalizedValue(pc,
                    x => x.Title,
                    localized.Title,
                    localized.LanguageId);
            }
        ...
        #region Product Content Types

        public virtual IActionResult ProductContentTypes() {
            if(!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedDataTablesJson();

            ProductContentTypeSearchModel model = new ProductContentTypeSearchModel();
            return View(model);
        }

        [HttpPost]
        public virtual IActionResult ProductContentTypesList(ProductContentTypeSearchModel searchModel) {
            var data = _productContentService.GetAllProductContentTypes(searchModel.Page - 1, searchModel.PageSize);
            var model = new ProductContentTypeListModel() {
                Data = data.Select(x => new ProductContentTypeModel {
                    Id = x.Id,
                    Name = x.Name
                }),
        ...
        public virtual IActionResult CreateEditProductContentType(int? id) {
            if(!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedDataTablesJson();

            ProductContentTypeModel model = new ProductContentTypeModel();
            Action<ProductContentTypeLocalizedModel, int> localizedModelConfiguration = null;

            if(id > 0) {
                var productContentType = _productContentService.GetProductContentTypeById(id.Value);
                if(productContentType != null) {
                    model.Name = productContentType.Name;
                    model.Id = productContentType.Id;

                    localizedModelConfiguration = (locale, languageId) => {
                        locale.Name = _localizationService.GetLocalized(productContentType, entity => entity.Name, languageId, false, false);
                    };
                }
            }
        ...
        [HttpPost]
        public virtual IActionResult CreateEditProductContentType(ProductContentTypeModel model) {
            if(ModelState.IsValid) {
                if(model.Id > 0) {
                    var productContentType = _productContentService.GetProductContentTypeById(model.Id);
                    if(productContentType != null) {
                        productContentType.Name = model.Name;
                        _productContentService.UpdateProductContentType(productContentType);
                        UpdateLocales(productContentType, model);

                        _notificationService.SuccessNotification("Admin.Product.ContentType.Updated");
                        return RedirectToAction("CreateEditProductContentType", new { id = productContentType.Id });
                    } else {
                        productContentType = new ProductContentType {
                            Name = model.Name
                        };
                        _productContentService.InsertProductContentType(productContentType);
                        UpdateLocales(productContentType, model);
                    ...
                    var productContentType = new ProductContentType {
                        Name = model.Name
                    };
                    _productContentService.InsertProductContentType(productContentType);
                    UpdateLocales(productContentType, model);

                    _notificationService.SuccessNotification("Admin.Product.ContentType.Added");
                    return RedirectToAction("CreateEditProductContentType", new { id = productContentType.Id });
                }
            }
        ...
        [HttpPost]
        public virtual IActionResult ProductContentTypeDelete(int Id) {
            var productContentType = _productContentService.GetProductContentTypeById(Id);
            _productContentService.DeleteProductContentType(productContentType);
        ...
        public virtual IActionResult ProductContents() {
            if(!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedDataTablesJson();

            ProductContentSearchModel model = new ProductContentSearchModel();

            model.ProductContentTypes = _productContentService.GetAllProductContentTypes(0, int.MaxValue).Select(x => new SelectListItem {
                Text = x.Name,
                Value = x.Id.ToString()
            }).ToList();
            model.ProductContentTypes.Insert(0, new SelectListItem { Text = "Select", Value = "0" });

            return View(model);
        }

        [HttpPost]
        public virtual IActionResult ProductContentsList(ProductContentSearchModel searchModel) {
            var data = _productContentService.GetAllProductContents(searchModel.Page - 1, searchModel.PageSize, searchModel.ProductContentTypeId, searchModel.SearchTitle);
            var model = new ProductContentListModel() {
                Data = data.Select(x => new ProductContentModel {
                    Id = x.Id,
                    Title = x.Title,
                    IsActive = x.IsActive,
                    Link = x.Link,
                    ProductContentTypeId = x.ProductContentTypeId,
                    ThumbPictureId = x.ThumbPictureId,
                    ProductContentTypeName = x.ProductContentTypeId > 0 ? _productContentService.GetProductContentTypeById(x.ProductContentTypeId).Name : ""
                }),
                RecordsTotal = data.TotalCount
            };
        ...
        public virtual IActionResult CreateEditProductContents(int? id) {
            if(!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedDataTablesJson();

            ProductContentModel model = new ProductContentModel();
            Action<ProductContentLocalizedModel, int> localizedModelConfiguration = null;
            ...
            if(id > 0) {
                var productContent = _productContentService.GetProductContentById(id.Value);
                if(productContent != null) {
                    model.Title = productContent.Title;
                    model.IsActive = productContent.IsActive;
                    model.Link = productContent.Link;
                    model.ProductContentTypeId = productContent.ProductContentTypeId;
                    model.ThumbPictureId = productContent.ThumbPictureId;
                    model.Id = productContent.Id;

                    localizedModelConfiguration = (locale, languageId) => {
                        locale.Title = _localizationService.GetLocalized(productContent, entity => entity.Title, languageId, false, false);
                    };
                }
            }
        ...
        [HttpPost]
        public virtual IActionResult CreateEditProductContents(ProductContentModel model) {

            if(model.ProductContentTypeId <= 0) {
                ModelState.AddModelError("ProductContentTypeId", "Please select content type");
            }

            if(ModelState.IsValid) {
                var id = model.Id;
                if(model.Id > 0) {
                    var productContent = _productContentService.GetProductContentById(model.Id);
                    if(productContent != null) {
                        productContent.Title = model.Title;
                        productContent.IsActive = model.IsActive;
                        productContent.Link = model.Link;
                        productContent.ProductContentTypeId = model.ProductContentTypeId;
                        productContent.ThumbPictureId = model.ThumbPictureId;

                        _productContentService.UpdateProductContent(productContent);
                        UpdateLocales(productContent, model);

                        _notificationService.SuccessNotification("Admin.Product.Contents.Updated");
                    } else {
                        productContent = new ProductContent {
                            Title = model.Title,
                            IsActive = model.IsActive,
                            Link = model.Link,
                            ProductContentTypeId = model.ProductContentTypeId,
                            ThumbPictureId = model.ThumbPictureId
                        };
                        _productContentService.InsertProductContent(productContent);
                    ...
                    _notificationService.SuccessNotification("Admin.Product.Contents.Added");
                }

                return RedirectToAction("CreateEditProductContents", new { id });
            }
        ...
        [HttpPost]
        public virtual IActionResult ProductContentDelete(int Id) {
            var productContent = _productContentService.GetProductContentById(Id);
            _productContentService.DeleteProductContent(productContent);
        ...
        #region Product Content products

        [HttpPost]
        public virtual IActionResult ProductContentMappingList(ProductContentMappingSearchModel searchModel) {
            if(!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedDataTablesJson();

            //try to get a product with the specified id
            var productContent = _productContentService.GetProductContentById(searchModel.ProductContentId)
                ?? throw new ArgumentException("No product content found with the specified id");

            //prepare model
            var data = _productContentService.GetProductContentMappingByContentId(productContent.Id, searchModel.Page - 1, searchModel.PageSize);
            var model = new ProductContentMappingListModel() {
                Data = data.Select(x => new ProductContentMappingModel {
                    Id = x.Id,
        ...
        [HttpPost]
        public virtual IActionResult ProductContentMappingDelete(int id) {
            if(!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();

            //try to get a related product with the specified id
            var productContentMapping = _productContentService.GetProductContentMappingById(id)
                ?? throw new ArgumentException("No related product content mapping found with the specified id");

            _productContentService.DeleteProductContentMapping(productContentMapping);

            return new NullJsonResult();
        }

        public virtual IActionResult ProductContentMappingAddPopup(int productContentId) {
            if(!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();
        ...
        [HttpPost]
        public virtual IActionResult ProductContentMappingAddPopupList(AddRelatedProductSearchModel searchModel) {
            if(!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedDataTablesJson();
        ...
        [HttpPost]
        [FormValueRequired("save")]
        public virtual IActionResult ProductContentMappingAddPopup(AddProductContentMappingModel model) {
            if(!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();

            var selectedProducts = _productService.GetProductsByIds(model.SelectedProductIds.ToArray());
            if(selectedProducts.Any()) {
                var existingProductMappings = _productContentService.GetProductContentMappingByContentId(model.ProductContentId, 0, int.MaxValue);
                foreach(var product in selectedProducts) {
                    //a vendor should have access only to his products
                    if(_workContext.CurrentVendor != null && product.VendorId != _workContext.CurrentVendor.Id)
                        continue;

                    if(existingProductMappings.Any(x => x.ProductId == product.Id))
                        continue;

                    _productContentService.InsertProductContentMapping(new ProductContentMapping {
        ...
        public virtual IActionResult InsertContentLocale() {
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.ContentTypes", "Product Content Types");
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.ContentType.Name", "Name");
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.ContentType.Name.Hint", "Name");
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.Contents", "Product Contents");
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.Content.ContentType", "Content Type");
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.Content.ContentType.Hint", "Content Type");
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.Content.Title", "Title");
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.Content.Title.Hint", "Title");
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.Content.IsActive", "Is Active");
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.Content.IsActive.Hint", "Is Active");
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.Content.Link", "Link");
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.Content.Link.Hint", "Link");
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.Content.ThumbPictureId", "Thumb Picture");
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.Content.ThumbPictureId.Hint", "Thumb Picture");
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.ContentTypes.AddNew", "Add New Product Content Type");
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.ContentTypes.Edit", "Edit Product Content Type");
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Catalog.ProductContentType.BackToList", "Back to list");
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.ContentTypes.Info", "Info");

            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.Contents.AddNew", "Add New Product Content");
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.Contents.Edit", "Edit Product Content");
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Catalog.Contents.BackToList", "Back to list");
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.Contents.Info", "Info");

            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.ContentType.Updated", "Product content type updated successfully");
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.ContentType.Added", "Product content type added successfully");
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.Contents.Updated", "Product content updated successfully");
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.Contents.Added", "Product content added successfully");
            _localizationService.AddOrUpdatePluginLocaleResource("Admin.Product.Contents.Products", "Products");

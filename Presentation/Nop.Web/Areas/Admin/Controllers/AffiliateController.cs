...
namespace Nop.Web.Areas.Admin.Controllers
{
    public partial class AffiliateController : BaseAdminController
    {
        ...
        public virtual IActionResult AffiliateTypes() {
            //prepare model
            var model = _affiliateModelFactory.PrepareAffiliateSearchModel(new AffiliateSearchModel());
            return View(model);
        }
        ...
        [HttpPost]
        public virtual IActionResult AffiliateTypeList(AffiliateSearchModel searchModel) {
            if(!_permissionService.Authorize(StandardPermissionProvider.ManageAffiliates))
                return AccessDeniedDataTablesJson();

            var affiliateTypes = _affiliateService.GetAllAffiliateTypes(searchModel.Page - 1, searchModel.PageSize);

            //prepare list model
            var model = new AffiliateTypeListModel().PrepareToGrid(searchModel, affiliateTypes, () => {
                //fill in model values from the entity
                return affiliateTypes.Select(affiliateType => {
                    var affiliateTypeModel = affiliateType.ToModel<AffiliateTypeModel>();
                    return affiliateTypeModel;
                });
            });
        ...
        public virtual IActionResult CreateUpdateAffiliateType(int? id) {
            if(!_permissionService.Authorize(StandardPermissionProvider.ManageAffiliates))
                return AccessDeniedView();
            ...
            if(id > 0) {
                var affiliateType = _affiliateService.GetAffiliateTypeById(id.Value);
                if(affiliateType != null) {
                    model = affiliateType.ToModel<AffiliateTypeModel>();
                    model.Id = affiliateType.Id;
                }
            }
        ...
        [HttpPost]
        public virtual IActionResult CreateUpdateAffiliateType(AffiliateTypeModel model) {
            if(!_permissionService.Authorize(StandardPermissionProvider.ManageAffiliates))
                return AccessDeniedView();
            ...
            if(ModelState.IsValid) {
                var affiliateType = model.ToEntity<AffiliateType>();

                if(model.Id > 0) {
                    affiliateType.Id = model.Id;
                    _affiliateService.UpdateAffiliateType(affiliateType);
                    _notificationService.SuccessNotification(_localizationService.GetResource("Admin.AffiliateType.Updated"));
                } else {
                    _affiliateService.InsertAffiliateType(affiliateType);
                    _notificationService.SuccessNotification(_localizationService.GetResource("Admin.AffiliateType.Created"));
                }


                return RedirectToAction("AffiliateTypes");
            }
        ...
        [HttpPost]
        public virtual IActionResult DeleteAffiliateType(int id) {
            if(!_permissionService.Authorize(StandardPermissionProvider.ManageAffiliates))
                return AccessDeniedView();
            ...
            _notificationService.SuccessNotification(_localizationService.GetResource("Admin.AffiliateType.Deleted"));

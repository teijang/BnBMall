    ...
    public partial class ProductReviewModelFactory : IProductReviewModelFactory
    {
        ...
        public virtual ProductReviewModel PrepareProductReviewModel(ProductReviewModel model,
            ...
            if(productReview != null) {
                var imageIdStrs = _genericAttributeService.GetAttribute<string>(productReview, "ReviewImageId");
                var reviewVideos = _genericAttributeService.GetAttribute<string>(productReview, "ReviewVideos");

                var imageIds = new List<string>();
                var videos = new List<string>();

                if(!string.IsNullOrEmpty(imageIdStrs)) {
                    imageIds = imageIdStrs.Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries).Select(x => x.Trim()).Where(x => x != "0").ToList();
                }
                ...
                foreach(var item in imageIds) {
                    ...
                    model.ImageUrl.Add(_pictureService.GetReviewPictureUrl(imageId, 200));
                }

                foreach(var item in videos) {
                    model.VideoUrls.Add(videoBucketCDNName + "/" + item);
                }

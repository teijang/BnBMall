    ...
    public partial class AffiliateModelFactory : IAffiliateModelFactory
    {
        ...
        public virtual AffiliateModel PrepareAffiliateModel(AffiliateModel model, Affiliate affiliate, bool excludeProperties = false)
        {
            ...
            if (affiliate != null)
            {
                ...
                var affiliateTypes = _affiliateService.GetAllAffiliateTypes(0, int.MaxValue);
                model.AffiliateTypes = affiliateTypes.Select(x => new SelectListItem {
                    Text = x.Name,
                    Value = x.Id.ToString()
                }).ToList();
                ...
                if (!excludeProperties)
                {
                    ...
                    model.CommissionRate = affiliate.CommissionRate;
                    model.AffiliateTypeId = affiliate.AffiliateTypeId;
                }
            }
            else
            {

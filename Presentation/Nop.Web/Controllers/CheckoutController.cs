...
namespace Nop.Web.Controllers
{
    ...
    public partial class CheckoutController : BasePublicController
    {
        ...
        protected virtual void SavePickupOption(PickupPoint pickupPoint, IFormCollection form = null)
        {
            ...
            if(form != null) {
                pickUpInStoreShippingOption.CheckInDate = form["CheckInDate"];
                pickUpInStoreShippingOption.CheckOutDate = form["CheckOutDate"];
                pickUpInStoreShippingOption.RoomNumber = form["RoomNumber"];
                pickUpInStoreShippingOption.ReservationName = form["ReservationName"];
            }
        ...
        public virtual IActionResult NewShippingAddress(CheckoutShippingAddressModel model, IFormCollection form)
        {
            ...
            if (_shippingSettings.AllowPickupInStore && !_orderSettings.DisplayPickupInStoreOnShippingMethodPage)
            {
                ...
                if (pickupInStore)
                {
                    var checkinDate = form["CheckInDate"];
                    var checkOutDate = form["CheckOutDate"];
                    var roomNumber = form["RoomNumber"];
                    var reservationName = form["ReservationName"];

                    if(form["AllowCheckInCheckOutSelection"].ToString() == "True") {
                        if(string.IsNullOrEmpty(checkinDate)) {
                            ModelState.AddModelError("CheckInDate", "Please enter check-in date");
                        }

                        if(string.IsNullOrEmpty(checkOutDate)) {
                            ModelState.AddModelError("checkOutDate", "Please enter check-out date");
                        }
                    }

                    if((!string.IsNullOrEmpty(checkinDate) && !string.IsNullOrEmpty(checkOutDate)) || form["AllowCheckInCheckOutSelection"].ToString() == "False") {
                        var pickupOption = ParsePickupOption(form);
                        SavePickupOption(pickupOption, form);

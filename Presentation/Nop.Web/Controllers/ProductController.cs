    ...
    public partial class ProductController : BasePublicController
    {
        ...
        public virtual IActionResult ProductReviewsAdd(int productId, ProductReviewsModel model, bool captchaValid) {
            ...
            if(ModelState.IsValid) {
                ...
                _genericAttributeService.SaveAttribute(productReview, "ReviewImageId", model.AddProductReview.ImageId);
        ...
        [IgnoreAntiforgeryToken]
        public ActionResult ProductReviewsList(int productId, int reviewId, bool success = false) {
            var product = _productService.GetProductById(productId);
            var model = new ProductReviewsModel();
            model = _productModelFactory.PrepareProductReviewsModel(model, product);
            if(reviewId > 0) {
                var productReview = _productService.GetProductReviewById(reviewId);
                model.AddProductReview.Title = productReview.Title;
                model.AddProductReview.ReviewText = productReview.ReviewText;
                model.AddProductReview.ReviewId = productReview.Id;
                //model.AddProductReview.ImageId = _genericAttributeService.GetAttribute<int>(productReview, "ReviewImageId");
                model.AddProductReview.Rating = productReview.Rating;
            }
            ...
            return PartialView("ProductReviewsList", model);
        }

        [HttpPost]
        [IgnoreAntiforgeryToken]
        public virtual IActionResult EditReview(ProductReviewsModel model) {
            var productReview = _productService.GetProductReviewById(model.AddProductReview.ReviewId);
            if(productReview != null) {
                productReview.ReviewText = model.AddProductReview.ReviewText;
                productReview.Rating = model.AddProductReview.Rating;
                _productService.UpdateProductReview(productReview);

                var imageIds = model.AddProductReview.ImageId.Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries).Select(x => x.Trim()).Where(x => x != "0");
                var imageIdsStr = "";
                if(imageIds.Count() > 0) {
                    imageIdsStr = string.Join(",", imageIds);
                }
                _genericAttributeService.SaveAttribute(productReview, "ReviewImageId", imageIdsStr);
            }

            var product = _productService.GetProductById(model.AddProductReview.ReviewProductId);
            _productService.UpdateProductReviewTotals(product);

            model = _productModelFactory.PrepareProductReviewsModel(model, product);
            model.AddProductReview.Title = null;
            model.AddProductReview.ReviewText = null;
            model.AddProductReview.ReviewId = 0;
            return PartialView("ProductReviewsList", model);
        }
        ...
        [HttpPost]
        [IgnoreAntiforgeryToken]
        public IActionResult AddQuestion(int productId, string question) {
            try {
                var productQA = new ProductQA {
                    AskedBy = _workContext.CurrentCustomer.Id,
                    IsApproved = false,
                    ProductId = productId,
                    Question = question,
                    StoreId = _storeContext.CurrentStore.Id,
                ...
                _productQAService.InsertProductQA(productQA);
                _workflowMessageService.SendNewQuestionPostMessage(_workContext.CurrentCustomer, productQA, _workContext.WorkingLanguage.Id);
                return Json(new { success = true, message = _localizationService.GetResource("ProductQA.QuestionAdded") });
            } catch(Exception ex) {
        ...
        [HttpPost]
        [IgnoreAntiforgeryToken]
        public virtual IActionResult AddProductReviews(ProductReviewsModel model) {
            var productId = model.AddProductReview.ReviewProductId;
            var product = _productService.GetProductById(productId);
            if(product == null || product.Deleted || !product.Published || !product.AllowCustomerReviews)
                return RedirectToRoute("Homepage");

            ModelState.Remove("AddProductReview.Title");
            if(_customerService.IsGuest(_workContext.CurrentCustomer) && !_catalogSettings.AllowAnonymousUsersToReviewProduct) {
                ModelState.AddModelError("", _localizationService.GetResource("Reviews.OnlyRegisteredUsersCanWriteReviews"));
            }
            ...
            if(ModelState.IsValid) {
                ...
                if(!string.IsNullOrEmpty(model.AddProductReview.ImageId)) {
                    var imageIds = model.AddProductReview.ImageId.Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries).Select(x => x.Trim()).Where(x => x != "0");
                    var imageIdsStr = "";
                    if(imageIds.Count() > 0) {
                        imageIdsStr = string.Join(",", imageIds);
                ...
                if(!string.IsNullOrEmpty(model.AddProductReview.Video)) {
                    var videoUrls = model.AddProductReview.Video.Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries).Select(x => x.Trim()).Where(x => !string.IsNullOrEmpty(x));
                    var videoUrlsStr = "";
                    if(videoUrls.Count() > 0) {
                        videoUrlsStr = string.Join(",", videoUrls);
                    }
                    _genericAttributeService.SaveAttribute(productReview, "ReviewVideos", videoUrlsStr);
                }
